...
public class LoomRepository(IUnitOfWork unitOfWork) : DapperRepository(unitOfWork), ILoomRepository
{
    ...
    public async Task<Result<IEnumerable<FilterOption>>> GetFilterOptionsAsync(CancellationToken cancellationToken = default)
    {
        ...
        if (loomsResult.IsSuccess && loomsResult.Data?.Any() == true)
        {
            ...
            // Hall Names
            var hallNames = looms.Where(l => !string.IsNullOrEmpty(l.HallName))
            ...
            // Mark Names
            var markNames = looms.Where(l => !string.IsNullOrEmpty(l.MarkName))
            ...
            // Group Names
            var groupNames = looms.Where(l => !string.IsNullOrEmpty(l.GroupName))
            ...
            // Model Names  
            var modelNames = looms.Where(l => !string.IsNullOrEmpty(l.ModelName))
            ...
            // Class Names
            var classNames = looms.Where(l => !string.IsNullOrEmpty(l.ClassName))
            ...
            // Event Names
            var eventNames = looms.Where(l => !string.IsNullOrEmpty(l.EventNameTR))
